...
object Anagrams {
  ...
  val dictionary: List[Word] =
    scala.io.Source.fromInputStream(ucu.resourceAsStream("words.txt").get).getLines.toList
  ...
  def hehe(occ: Occurrences):List[Sentence] = occ match {
    ...
    case _ =>
      ...
      for {
        ...
        rest <- hehe(subtract(occ, comb)) if occ.nonEmpty
      } yield word::rest
  }
}